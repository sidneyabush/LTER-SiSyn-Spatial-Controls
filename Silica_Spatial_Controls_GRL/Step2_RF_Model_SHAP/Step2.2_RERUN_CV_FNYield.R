# #############################################################################
# Re-run RF predictions for UPDATED Cross-Validation (unseen10) data - FNYield
# #############################################################################
# Purpose:
...
#   2) <out_dir>/FNYield_RF2_model_and_settings_split.RData (existing RF2 model)
#   3) <out_dir>/Predictions_FNYield_split.csv (existing predictions)
#
# Outputs updated:
#   A) <out_dir>/Predictions_FNYield_split.csv (unseen10 rows replaced)
# #############################################################################
...
load(file.path(output_dir, "FNYield_RF2_model_and_settings_split.RData"))
# This loads: rf2_FNYield, nt2_FNYield, mtry2_FNYield, feats_FNYield

# 3) Load UPDATED unseen10 data (using same preprocessing as Step 2.2)
...
# Apply same load_split function from original Step 2.2
df_unseen10 <- df_temp %>%
...
new_unseen10_preds <- tibble(
  subset    = "unseen10",
  observed  = df_unseen10_clean$FNYield,
  predicted = predict(rf2_FNYield, df_unseen10_clean)
)
...
pred_df_old <- read.csv(file.path(output_dir, "Predictions_FNYield_split.csv"))
...
cat("\n✓ Successfully updated Predictions_FNYield_split.csv\n")
