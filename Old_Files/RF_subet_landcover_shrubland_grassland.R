...
## Land Cover
drivers_df <- drivers_df[drivers_df$major_land %like% "shrub",]
...
rf_model1<-randomForest(med_si~.,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=300,mtry=5)
...
rf_model2<-randomForest(rf_formula,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=700, mtry=3)
...
lm_plot <- plot(rf_model2$predicted, drivers_df$med_si, xlab="Predicted", ylab="Observed", 
                main= "Optimized RF Model - Shrubland-Grassland") + abline(a=0, b=1, col="red") + theme(text = element_text(size=20))
