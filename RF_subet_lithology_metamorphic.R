...
## Lithology
drivers_df <- drivers_df[drivers_df$major_rock %like% "metamorphic",]
...
rf_model1<-randomForest(med_si~.,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=1000,mtry=8)
...
rf_model2<-randomForest(rf_formula,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=800, mtry=1)
...
lm_plot <- plot(rf_model2$predicted, drivers_df$med_si, xlab="Predicted", ylab="Observed", 
                main= "Optimized RF Model - Metamorphic Rocks") + abline(a=0, b=1, col="red") + theme(text = element_text(size=20))
