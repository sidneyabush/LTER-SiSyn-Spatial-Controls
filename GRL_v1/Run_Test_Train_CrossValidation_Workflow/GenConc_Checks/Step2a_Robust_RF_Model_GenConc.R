# =============================================================================
# Train RF for GenConc: older70 (training) → RF1 → stability → RF2 → predict on recent30 & unseen10
# =============================================================================
...
# =============================================================================
# a) RF1 tuning for GenConc
# =============================================================================
df_tr_full <- df_train %>%
  drop_na(all_of(c("GenConc", predictors)))
x1 <- df_tr_full[predictors]
y1 <- df_tr_full$GenConc

# scan ntree
mse1 <- test_numtree_parallel(tree_grid, as.formula("GenConc ~ ."), df_tr_full)
...
rf1_GenConc  <- rf1
imps_GenConc <- randomForest::importance(rf1)[, "%IncMSE"]
save(rf1_GenConc, imps_GenConc,
     file = file.path(output_dir, "GenConc_RF1.RData"))

# =============================================================================
# b) Stability selection
# =============================================================================
imp_thr_GenConc  <- quantile(imps_GenConc, 0.50)
freq_thr_GenConc <- 0.80

stab_GenConc <- rf_stability_selection_parallel(
  x                     = df_train[predictors],
  y                     = df_train$GenConc,
  n_bootstrap           = 500,
  threshold             = freq_thr_GenConc,
  ntree                 = rf1_GenConc$ntree,
  mtry                  = rf1_GenConc$mtry,
  importance_threshold  = imp_thr_GenConc
)
feats_GenConc <- stab_GenConc$features

# save stability outputs
save(stab_GenConc, feats_GenConc, imp_thr_GenConc, freq_thr_GenConc,
     file = file.path(output_dir, "GenConc_stability_selection.RData"))
write.csv(
  tibble(variable = names(stab_GenConc$frequencies),
         frequency = stab_GenConc$frequencies),
  file      = file.path(output_dir, "GenConc_stability_frequencies.csv"),
  row.names = FALSE
)
...
# =============================================================================
df2_GenConc <- df_train %>%
  drop_na(all_of(c("GenConc", feats_GenConc))) %>%
  select(GenConc, all_of(feats_GenConc))

mse2_GenConc <- test_numtree_parallel(tree_grid, as.formula("GenConc ~ ."), df2_GenConc)
nt2_GenConc  <- tree_grid[which.min(mse2_GenConc)]

t2_GenConc   <- randomForest::tuneRF(
  x        = df2_GenConc[feats_GenConc],
  y        = df2_GenConc$GenConc,
  ntreeTry = nt2_GenConc,
  stepFactor = 1.5,
  improve    = 0.01,
  plot       = FALSE
)
mtry2_GenConc <- t2_GenConc[which.min(t2_GenConc[,2]),1]

rf2_GenConc   <- randomForest(
  x          = df2_GenConc[feats_GenConc],
  y          = df2_GenConc$GenConc,
  ntree      = nt2_GenConc,
  mtry       = mtry2_GenConc,
...
save(rf2_GenConc, nt2_GenConc, mtry2_GenConc, feats_GenConc,
     file = file.path(output_dir, "GenConc_RF2_model_and_settings.RData"))

save(rf2_GenConc, 
     file = file.path(output_dir, "GenConc_Yearly_rf_model2.RData"))

# =============================================================================
# d) Export kept_drivers for SHAP
# =============================================================================
kept_drivers_GenConc <- df_recent30 %>%
  drop_na(all_of(c("GenConc", feats_GenConc))) %>%
  select(all_of(feats_GenConc))

save(kept_drivers_GenConc,
     file = file.path(output_dir, "GenConc_Yearly_kept_drivers.RData"))

# =============================================================================
# e) Diagnostics & plots
# =============================================================================
save_correlation_plot(cor(df_train[predictors]), output_dir, "GenConc")
save_rf_importance_plot(rf2_GenConc, output_dir, "GenConc")
save_lm_plot(rf2_GenConc, df2_GenConc$GenConc, output_dir, "GenConc")

pred_list_GenConc <- list(
  older70 = tibble(subset   = "older70",
                   observed = df2_GenConc$GenConc,
                   predicted= predict(rf2_GenConc, df2_GenConc)),
  recent30 = tibble(subset   = "recent30",
                    observed = df_recent30$GenConc,
                    predicted= predict(rf2_GenConc,
                                       df_recent30 %>%
                                         drop_na(all_of(c("GenConc", feats_GenConc))))),
  unseen10 = tibble(subset   = "unseen10",
                    observed = df_unseen10$GenConc,
                    predicted= predict(rf2_GenConc,
                                       df_unseen10 %>%
                                         drop_na(all_of(c("GenConc", feats_GenConc)))))
)
pred_df_GenConc <- bind_rows(pred_list_GenConc)

write.csv(pred_df_GenConc,
          file      = file.path(output_dir, "Predictions_GenConc.csv"),
          row.names = FALSE)

save_rf2_all_subsets_plot(pred_df_GenConc, "GenConc", output_dir)

# =============================================================================
# f) Save stability + importance summary (with header comment)
# =============================================================================
stab_df <- tibble(
  variable  = names(stab_GenConc$frequencies),
  frequency = stab_GenConc$frequencies,
  incMSE    = imps_GenConc[names(stab_GenConc$frequencies)],
  selected  = names(stab_GenConc$frequencies) %in% feats_GenConc
)

stab_out <- file.path(output_dir, "GenConc_08_Feature_Stability_and_medianImportance.csv")
writeLines(
  sprintf("# Importance threshold (median %%IncMSE from RF1) = %.5f", imp_thr_GenConc),
  con = stab_out
)
write.table(
  stab_df,
