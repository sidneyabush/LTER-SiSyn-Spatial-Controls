...
si_stats <- wrtds_df %>%
  ...
  dplyr::summarise(GenConc=median(GenConc))
...
# chem_df <- wrtds_df[,c("LTER.x", "Stream_ID", "chemical", "DecYear","GenConc", "FNFlux")]
...
#   # remove zero values for flow and Si GenConc
#   tot <- tot[tot$Q > 0,]
#   tot <- tot[tot$GenConc > 0,]
#   # calculate CQ
#   lm1 <- lm(log(GenConc)~log(Q), tot)
#   sum <- summary(lm1)
...
# write.csv(tot, "AllDrivers_Harmonized_20241108_WRTDS_MD_KG_rawNP_GenConc_Average.csv")
