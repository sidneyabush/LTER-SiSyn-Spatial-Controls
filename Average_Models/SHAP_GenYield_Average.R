...
create_all_shapley_plots <- function(shap_values, output_file) {
  ...
  overall_importance_plot <- ggplot(overall_feature_importance, aes(x = reorder(feature, importance), y = importance)) +
    ...
    coord_flip() +
    labs(x = "Feature", y = "Mean Absolute SHAP Value", title = "Ave GenYield Overall Feature Importance") +
    theme_minimal()
...
create_shap_partial_dependence_plots <- function(shap_values, kept_drivers, train, output_dir, color_var = "GenYield") {
  ...
  for (feature in colnames(shap_values)) {
    ...
    shap_pdp_plot <- ggplot(shap_long, aes(x = feature_value, y = shap_value, color = color_value)) +
      ...
      labs(
        title = paste("Ave GenYield SHAP Partial Dependence Plot for", feature),
