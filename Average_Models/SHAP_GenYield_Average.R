...
load("GenYield_Average_full.RData")
...
create_all_shapley_plots <- function(shap_values, output_file) {
  ...
  overall_importance_plot <- ggplot(overall_feature_importance, aes(x = reorder(feature, importance), y = importance)) +
    ...
    coord_flip() +
    labs(x = "Feature", y = "Mean Absolute SHAP Value", title = "GenYield Average - Overall Feature Importance") +
    theme_minimal() +
    theme(
...
output_file <- sprintf("%s/SHAP_GenYield_Ave_Overall_Variable_Importance.pdf", output_dir)
