...
test_numtree_average <- function(ntree_list) {
  ...
  for (i in 1:length(ntree_list)) {
    ...
    set.seed(123)
    rf_model <- randomForest(GenYield ~ ., data = drivers_df, importance = TRUE, proximity = TRUE, ntree = ntree_list[[i]])
    MSE[[i]] <- rf_model$mse
  }
  return(MSE)
}
...
save_rf_importance_plot <- function(rf_model, output_dir) {
  ...
  randomForest::varImpPlot(rf_model, main = "RF Variable Importance - Average Gen Yield", col = "darkblue")
  dev.off()
}
...
save_lm_plot <- function(rf_model, observed, output_dir) {
  ...
  plot(rf_model$predicted, observed, pch = 16, cex = 1.5,
       xlab = "Predicted", ylab = "Observed", main = "Observed vs Predicted - Average Gen Yield",
...
drivers_df <- read.csv("AllDrivers_Harmonized_Average.csv") %>%
  select(-contains("Conc"), -contains("FN"), -contains("Q"), -drainage_area) %>%
...
title("Median Gen Si Yield")  # Add title in the PDF
dev.off()
...
set.seed(123)
rf_model1 <- randomForest(GenYield ~ ., data = drivers_df, importance = TRUE, proximity = TRUE, ntree = 1000, mtry = 9)
...
lm_plot <- plot(rf_model2$predicted, drivers_df$GenYield, pch = 16, cex = 1.5,
                xlab = "Predicted", ylab = "Observed", main = "All Spatial Drivers - Gen Yield",
...
save(rf_model2, file = "GenYield_Ave_rf_model2.RData")
kept_drivers <- drivers_df[, colnames(drivers_df) %in% predictors(result_rfe)]
save(kept_drivers, file = "GenYield_Ave_kept_drivers.RData")
save(drivers_df, file = "GenYield_Ave_drivers_df.RData")
