...
chem_NP <- subset(chem_NP, chem_NP$GenFlux > 0)

## ------------------------------------------------------- ##
# Calculate median N and P values ----
## ------------------------------------------------------- ##
## this is for Flux (GenFlux) 
chem_NP_avg <- chem_NP %>%
  dplyr::group_by(Stream_Name, chemical) %>%
  dplyr::summarise(median_Flux = median(GenFlux, na.rm = T))
...
# Export .csv:
write.csv(chem_NP_avg, "Median_NP_WRTDS_GenFlux_2.csv")