...
# Import raw P data, renaming the P column for clarity
raw_P <- read.csv("AllDrivers_Harmonized_20241108_WRTDS_MD_KG_rawNP_GenFlux.csv") %>%
  ...
  dplyr::rename(raw_P = P)  # Rename the P column to raw_P for distinction
...
drivers_df <- drivers_df %>%
  ...
  mutate(P = ifelse(is.na(P_new), P, P_new)) %>%  # Replace P with P_new where available
