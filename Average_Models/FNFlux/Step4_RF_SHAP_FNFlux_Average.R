...
drivers_df <- read.csv("AllDrivers_Harmonized_20241108_WRTDS_MD_KG_NP_FNFlux_silicate_weathering.csv") %>%
...
num_predictors <- ncol(kept_drivers)

# Conditionally run tuneRF if there is more than one predictor
if (num_predictors > 1) {
  tuneRF(kept_drivers, drivers_df[, 1], ntreeTry = 2000, stepFactor = 1, improve = 0.5, plot = FALSE)
} else {
  # Print a message to confirm skipping tuneRF due to only one predictor
  print("Skipping tuneRF as there is only one predictor. Setting mtry = 1.")
  # Set mtry = 1 manually, as it is the only sensible choice with one predictor
}
