...
chem_NP <- subset(chem_NP, chem_NP$FNFlux > 0)

## ------------------------------------------------------- ##
# Calculate median N and P values ----
## ------------------------------------------------------- ##
## this is for Flux (FNFlux) 
chem_NP_avg <- chem_NP %>%
  dplyr::group_by(Stream_Name, chemical) %>%
  dplyr::summarise(median_Flux = median(FNFlux, na.rm = T))
...
# Export .csv:
write.csv(chem_NP_avg, "Median_NP_WRTDS_FNFlux_2.csv")