...
save_correlation_plot <- function(driver_cor, output_dir) {
  pdf(sprintf("%s/correlation_plot_GenYield_Average_5_years_noWeathering.pdf", output_dir), width = 10, height = 10)
...
save_rf_importance_plot <- function(rf_model, output_dir) {
  pdf(sprintf("%s/RF_variable_importance_GenYield_Average_5_years_noWeathering.pdf", output_dir), width = 8, height = 6)
...
save_lm_plot <- function(rf_model2, observed, output_dir) {
  pdf(sprintf("%s/RF2_lm_plot_GenYield_Average_5_years_drivers_numeric_noWeathering.pdf", output_dir), width = 8, height = 8)
  plot(rf_model2$predicted, observed, pch = 16, cex = 1.5,
       xlab = "Predicted", ylab = "Observed", main = "RF Model 2 Full Data Ave GenYield",
...
# ---- Remove Outliers for GenYield (3 SD Rule) ----
GenYield_mean <- mean(drivers_df$GenYield, na.rm = TRUE)
SD_val <- 3
GenYield_sd <- sd(drivers_df$GenYield, na.rm = TRUE)
GenYield_upper <- GenYield_mean + SD_val * GenYield_sd
GenYield_lower <- GenYield_mean - SD_val * GenYield_sd

drivers_df <- drivers_df %>%
  filter(GenYield >= GenYield_lower & GenYield <= GenYield_upper)
...
set.seed(123)
rf_model1 <- randomForest(GenYield ~ ., data = drivers_numeric, importance = TRUE, proximity = TRUE, ntree = manual_ntree_rf1, mtry = manual_mtry_rf1)
...
lm_plot <- plot(rf_model1$predicted, drivers_numeric$GenYield, pch = 16, cex = 1.5,
                xlab = "Predicted", ylab = "Observed", main = "RF Model 1 Full Data - Ave GenYield",
...
x <- drivers_numeric[, !(colnames(drivers_numeric) == "GenYield")]
y <- drivers_numeric$GenYield
...
save_lm_plot(rf_model2, drivers_numeric$GenYield, output_dir)


# Save model and required objects for SHAP analysis
save(rf_model2, file = "GenYield_Average_rf_model2_full.RData")
kept_drivers <- drivers_numeric[, colnames(drivers_numeric) %in% predictors(result_rfe)]
save(kept_drivers, file = "GenYield_Average_kept_drivers_full.RData")
save(drivers_numeric, file = "GenYield_Average_full.RData")
save(drivers_df, file = "GenYield_Average_full_stream_ids.RData")
