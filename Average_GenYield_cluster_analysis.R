...
load("GenYield_Average_kept_drivers_full.RData")
load("GenYield_Average_full_stream_ids.RData")
load("GenYield_Average_rf_model2_full.RData")
...
drivers_subset <- drivers_df %>% select(Stream_ID, GenYield)
...
dist <- ggplot(all_data, aes(x = cluster, y = GenYield, fill = cluster)) +
...
# Now remove GenYield and Stream_ID columns to put into SHAP analysis
combined_data <- all_data %>%
  dplyr::select(-GenYield, -Stream_ID)
