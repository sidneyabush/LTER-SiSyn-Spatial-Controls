...
shapley_list <- lapply(1:nrow(kept_drivers), function(i) {
  shap <- Shapley$new(predictor, x.interest = kept_drivers[i, ])  # Calculate for all variables
...
# Step 3: Reshape kept_drivers using melt from reshape2 to match the structure of shapley_df
kept_drivers_melted <- reshape2::melt(kept_drivers)
...
# Ensure that feature names and row indices match between shapley_df and kept_drivers_melted
shapley_plot_data <- shapley_df %>%
  mutate(row_id = rep(1:nrow(kept_drivers), each = ncol(kept_drivers))) %>%  # Add row_id to shapley_df
...
# Step 7: SHAP Partial Dependence Plots (PDPs)
# Function to generate PDP for each feature
plot_shap_dependence_iml <- function(predictor, feature) {
  pdp <- FeatureEffect$new(predictor, feature = feature, method = "pdp")  # Partial dependence plot
  print(pdp$plot())  # Print each PDP
}
