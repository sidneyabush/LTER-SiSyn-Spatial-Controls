...
## Land Cover
drivers_df <- drivers_df[drivers_df$major_land %like% "broadleaf",]
...
rf_model1<-randomForest(med_si~.,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=100,mtry=5)
...
set.seed(123)
tuneRF(kept_drivers, drivers_df[,1], ntreeTry = 100, stepFactor = 1, improve = 0.5, plot = FALSE)

#run optimized random forest model, with retuned ntree and mtry parameters
set.seed(123)
rf_model2<-randomForest(rf_formula,
                        data=drivers_df, importance=TRUE, proximity=TRUE, ntree=100, mtry=4)
...
lm_plot <- plot(rf_model2$predicted, drivers_df$med_si, xlab="Predicted", ylab="Observed", 
                main= "Optimized RF Model - Broadleaf Forest") + abline(a=0, b=1, col="red") + theme(text = element_text(size=20))
