...
###############################################################################
# 6. Create Long-format Data for Box Plots (exclude FNYield & rocks*)
###############################################################################
long_data <- scaled_data %>%
  ...
  # FILTER OUT "FNYield" and anything starting with "rocks"
  filter(
    Driver != "FNYield",
...
plot_mean_abs_shap <- function(cluster_id, shap_values_FNYield, full_scaled) {
  ...
  df_shap <- data.frame(
    ...
    # REMOVE "FNYield" and anything starting with "rocks"
    filter(
      feature != "FNYield",
...
generate_shap_dot_plot_obj <- function(cluster_name, shap_values_FNYield, full_scaled,
  ...
  cat("SHAP columns:", colnames(shap_values_FNYield_df), "\n")
  ...
  # (4) Remove any "rocks…" features and FNYield
  shap_long <- shap_long %>%
    filter(
      !grepl("^rocks", feature, ignore.case = TRUE),
      feature != "FNYield"
...
###############################################################################
ggsave(
  filename = "Fig4_FNYield_Cluster_Boxplot_SHAP_DotPlots.png",
...
ggsave(
  filename = "Fig5_FNYield_Yearly_Clusters.png",
  plot     = p_FNYield,
  width    = 8,
  height   = 5,
  dpi      = 300,
