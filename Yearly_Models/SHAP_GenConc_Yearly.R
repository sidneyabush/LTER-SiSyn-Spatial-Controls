...
librarian::shelf(iml, ggplot2, dplyr, tidyr, reshape2, parallel, foreach, doParallel)
...
create_shapley_plot_data <- function(model, kept_drivers, drivers_df, sample_size = 30) {
  ...
  shapley_list <- foreach(
    i = 1:nrow(kept_drivers),
    .packages = c("iml", "dplyr"),
    .export = c("predictor", "kept_drivers")
  ) %dopar% {
...
create_shap_partial_dependence_plots <- function(shap_data, drivers_df, output_dir, color_var = "GenConc") {
