...
load(file.path(output_dir, "FNConc_RF1.RData"))  # provides rf1 and imps
rf1_FNConc  <- rf1
imps_FNConc <- imps
rm(rf1, imps)
...
save(
  stab_FNConc, feats_FNConc, imp_thr_FNConc, freq_thr_FNConc,
  file = file.path(output_dir, "FNConc_stability_selection.RData")
)
write.csv(
  tibble(variable = names(stab_FNConc$frequencies), frequency = stab_FNConc$frequencies),
...
df2_FNConc <- df_train %>%
  ...
  select(all_of(c("FNConc", feats_FNConc)))
...
# Save RF2 model and tuning parameters
save(
  rf2_FNConc, nt2_FNConc, mtry2_FNConc, feats_FNConc,
...
pred_list_FNConc <- list(
  older70 = tibble(subset = "older70", observed = df2_FNConc$FNConc, predicted = predict(rf2_FNConc, df2_FNConc)),
  recent30 = tibble(subset = "recent30", observed = df_recent30$FNConc,
                    predicted = predict(rf2_FNConc, df_recent30 %>% drop_na(all_of(c("FNConc", feats_FNConc))))),
  unseen10 = tibble(subset = "unseen10", observed = df_unseen10$FNConc,
                    predicted = predict(rf2_FNConc, df_unseen10 %>% drop_na(all_of(c("FNConc", feats_FNConc)))))
)
