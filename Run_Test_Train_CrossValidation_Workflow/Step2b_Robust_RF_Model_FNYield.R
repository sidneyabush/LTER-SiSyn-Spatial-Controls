# Train RF for FNYield: older70 (training) → RF1 → stability → RF2 → predict on recent30 (testing) & unseen10 (cross-validation)
...
save_correlation_plot <- function(driver_cor, output_dir) {
  png(sprintf("%s/FNYield_Yearly_5yrs_corrplot.png", output_dir),
...
save_lm_plot <- function(rf_model2, observed, output_dir) {
  ...
  png(sprintf("%s/RF2_lm_plot_FNYield_Yearly_5_years.png", output_dir),
...
# FNYield pipeline
# =============================================================================
# a) Load cached RF1 and rename
load(file.path(output_dir, "FNYield_RF1.RData"))  # provides rf1 and imps
rf1_FNYield  <- rf1
imps_FNYield <- imps
rm(rf1, imps)
...
# Save stability output
save(
  stab_FNYield, feats_FNYield, imp_thr_FNYield, freq_thr_FNYield,
  file = file.path(output_dir, "FNYield_stability_selection.RData")
)
write.csv(
  tibble(variable = names(stab_FNYield$frequencies), frequency = stab_FNYield$frequencies),
...
df2_FNYield <- df_train %>%
  ...
  select(all_of(c("FNYield", feats_FNYield)))
...
# Save RF2 model and tuning parameters
save(
  rf2_FNYield, nt2_FNYield, mtry2_FNYield, feats_FNYield,
...
pred_list_FNYield <- list(
  older70 = tibble(subset = "older70", observed = df2_FNYield$FNYield, predicted = predict(rf2_FNYield, df2_FNYield)),
  recent30 = tibble(subset = "recent30", observed = df_recent30$FNYield,
                    predicted = predict(rf2_FNYield, df_recent30 %>% drop_na(all_of(c("FNYield", feats_FNYield))))),
  unseen10 = tibble(subset = "unseen10", observed = df_unseen10$FNYield,
                    predicted = predict(rf2_FNYield, df_unseen10 %>% drop_na(all_of(c("FNYield", feats_FNYield)))))
)
