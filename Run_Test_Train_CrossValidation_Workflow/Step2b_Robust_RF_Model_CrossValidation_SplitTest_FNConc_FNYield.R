...
# 3) ntree scan via foreach + randomForest (no progress bar)
test_numtree_parallel <- function(tree_grid, formula, df) {
...
# 4) stability‑selection via foreach (no progress bar)
...
for(resp in c("FNConc","FNYield")) {
  ...
  # RF1 tuning
  formula1 <- as.formula(paste(resp, "~ ."))
  mse1     <- test_numtree_parallel(tree_grid, formula1, df_tr)
  ...
  rf2   <- randomForest::randomForest(formula2, data=df2, ntree=nt2, mtry=mtry2, importance=TRUE)
...
message("✅ Training & evaluation complete.")
