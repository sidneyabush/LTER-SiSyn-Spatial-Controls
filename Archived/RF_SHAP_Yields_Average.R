...
save_rf_importance_plot <- function(rf_model, output_dir) {
  ...
  randomForest::varImpPlot(rf_model, main = "RF Variable Importance - Average Yield", col = "darkblue")
  dev.off()
}
...
save_lm_plot <- function(rf_model, observed, output_dir) {
  ...
  plot(rf_model$predicted, observed, pch = 16, cex = 1.5,
       xlab = "Predicted", ylab = "Observed", main = "Observed vs Predicted - Average Yield",
...
output_dir <- "/Users/sidneybush/Library/CloudStorage/Box-Box/Sidney_Bush/SiSyn/Figures/Average_Model/Yield"

# Read in and tidy data ----
# Set working directory
setwd("/Users/sidneybush/Library/CloudStorage/Box-Box/Sidney_Bush/SiSyn")

drivers_df <- read.csv("AllDrivers_Harmonized_20241106_WRTDS_MD_KG_NP_Yield_silicate_weathering.csv") %>%
...
create_all_shapley_plots <- function(shap_data, output_file, color_vars = NULL) {  # Accept color_vars as an argument
  ...
  overall_importance_plot <- ggplot(overall_feature_importance, aes(x = reorder(feature, importance), y = importance)) +
    ...
    coord_flip() +
    labs(x = "Feature", y = "Mean Absolute SHAP Value", title = "Overall Feature Importance - Average Yield") +
    theme_minimal()
  ...
  shap_summary_plot <- ggplot(shap_data_normalized, aes(x = phi, y = feature)) + 
    ...
    labs(x = "SHAP Value", y = NULL, title = "SHAP Summary Plot - Average Yield") + 
    theme_bw() + 
  ...
  pos_neg_plot <- ggplot(pos_neg_summary, aes(x = feature, y = mean_phi, fill = mean_phi > 0)) +
    ...
    labs(x = "Feature", y = "Mean SHAP Value", title = "Overall SHAP Impact by Feature - Average Yield") +
    coord_flip() +
    theme_minimal()
  ...
  for (feature_name in unique(shap_data$feature)) {
    for (color_var in color_vars) {
      ...
      dependence_plot <- ggplot(shap_data[shap_data$feature == feature_name, ], aes_mapping) +
        ...
        labs(x = paste("Value of", feature_name), y = "SHAP Value", title = paste("Average Yield SHAP Dependence Plot for", feature_name)) + 
        theme_minimal() +
